...
void preferences::conectar_signal(void)
{
	g_signal_connect (G_OBJECT(window), "delete_event",
												G_CALLBACK(preferences_delete_event_callback),
												this);

	// para ver si se cambio de estado spinner_idle_*
	g_signal_connect (G_OBJECT(spinner_idle_h), "changed", 
                      G_CALLBACK (preferences_idletime_callback), this);
	g_signal_connect (G_OBJECT(spinner_idle_m), "changed", 
                      G_CALLBACK (preferences_idletime_callback), this);
	g_signal_connect (G_OBJECT(spinner_idle_s), "changed", 
                      G_CALLBACK (preferences_idletime_callback), this);
/*
	g_signal_connect (G_OBJECT(&GTK_SPIN_BUTTON(spinner_idle_h)->entry), "key_release_event", 
                      G_CALLBACK (preferences_idletime_callback), this);
	g_signal_connect (G_OBJECT(&GTK_SPIN_BUTTON(spinner_idle_m)->entry), "key_release_event", 
                      G_CALLBACK (preferences_idletime_callback), this);
	g_signal_connect (G_OBJECT(&GTK_SPIN_BUTTON(spinner_idle_s)->entry), "key_release_event", 
                      G_CALLBACK (preferences_idletime_callback), this);
*/
	// para ver si se cambio de estado spinner_online_*
	g_signal_connect (G_OBJECT(spinner_online_h), "changed", 
                      G_CALLBACK (preferences_onlinetime_callback), this);
	g_signal_connect (G_OBJECT(spinner_online_m), "changed", 
                      G_CALLBACK (preferences_onlinetime_callback), this);
	g_signal_connect (G_OBJECT(spinner_online_s), "changed", 
                      G_CALLBACK (preferences_onlinetime_callback), this);
/*
	g_signal_connect (G_OBJECT(&GTK_SPIN_BUTTON(spinner_online_h)->entry), "key_release_event", 
                      G_CALLBACK (preferences_onlinetime_callback), this);
	g_signal_connect (G_OBJECT(&GTK_SPIN_BUTTON(spinner_online_m)->entry), "key_release_event", 
                      G_CALLBACK (preferences_onlinetime_callback), this);
	g_signal_connect (G_OBJECT(&GTK_SPIN_BUTTON(spinner_online_s)->entry), "key_release_event", 
                      G_CALLBACK (preferences_onlinetime_callback), this);
*/
...
//                      G_CALLBACK (preferences_combo_callback), this);

	// Interface
	g_signal_connect (G_OBJECT(spinner_prefered), "changed", 
                      G_CALLBACK (preferences_spinner_prefered_callback), this);

	g_signal_connect (G_OBJECT(button_refresh), "clicked",
											G_CALLBACK(preferences_refresh_callback), this);

	g_signal_connect(G_OBJECT(clist_interface), "select_row",
                  	 G_CALLBACK(preferences_select_row_callback), this);

	// botones OK | CANCEL
	g_signal_connect (G_OBJECT(button_ok), "clicked",
											G_CALLBACK(preferences_ok_callback),	this);

	g_signal_connect (G_OBJECT(button_cancel), "clicked",
											G_CALLBACK(preferences_cancel_callback),	this);

	// max byte in, max byte out
	g_signal_connect (G_OBJECT(spinner_byte_in), "changed", 
                      G_CALLBACK (preferences_byte_in_callback), this);

	g_signal_connect (G_OBJECT(spinner_byte_out), "changed", 
                      G_CALLBACK (preferences_byte_out_callback), this);

	// warnnings
	g_signal_connect(G_OBJECT(button_option_warn), "toggled",
	                   G_CALLBACK(preferences_button_option_warn_callback), this);

	g_signal_connect(G_OBJECT(spinner_warntime), "changed",
	                   G_CALLBACK(preferences_spinner_warntime_callback), this);

	g_signal_connect(G_OBJECT(button_warn_beep), "toggled",
	                   G_CALLBACK(preferences_button_warn_beep_callback), this);

	// start up options 
	g_signal_connect (G_OBJECT(idletime_on_button), "toggled",
                      G_CALLBACK (preferences_idletime_button_callback), this);

	g_signal_connect (G_OBJECT(onlinetime_on_button), "toggled",
                      G_CALLBACK (preferences_onlinetime_button_callback), this);
...
void preferences::cargar_notebook(void)
{
	...
	vbox_frame = gtk_box_new(GTK_ORIENTATION_VERTICAL, 5);
...
void preferences::cargar_spinner_idle(GtkWidget *vbox)
{
	...
	gtk_widget_set_size_request(spinner_idle_s, 40, 0);
	...
	gtk_widget_set_size_request(spinner_idle_m, 40, 0);
	...
	gtk_widget_set_size_request(spinner_idle_h, 65, 0);
	...
	gtk_widget_set_tooltip_text(spinner_idle_h, "hours");
	gtk_widget_set_tooltip_text(spinner_idle_m, "minutes");
	gtk_widget_set_tooltip_text(spinner_idle_s, "seconds");
...
void preferences::cargar_spinner_online(GtkWidget *vbox)
{
	...
	gtk_widget_set_size_request(spinner_online_s, 40, 0);
	...
	gtk_widget_set_size_request(spinner_online_m, 40, 0);
	...
	gtk_widget_set_size_request(spinner_online_h, 65, 0);
	...
	gtk_widget_set_tooltip_text(spinner_online_h, "hours");
	gtk_widget_set_tooltip_text(spinner_online_m, "minutes");
	gtk_widget_set_tooltip_text(spinner_online_s, "seconds");
...
void preferences::cargar_byte_in(GtkWidget *vbox) 
{
	...
	gtk_widget_set_size_request(spinner_byte_in, 65, 22);
...
void preferences::cargar_byte_out(GtkWidget *vbox) 
{
	...
	gtk_widget_set_size_request(spinner_byte_out, 65, 22);
...
void preferences::cargar_interface(GtkWidget *vbox)
{
	...
	gtk_scrolled_window_set_policy(GTK_SCROLLED_WINDOW (scrolled_win),
  															 ...
  															 GTK_POLICY_AUTOMATIC);
	gtk_widget_set_size_request(scrolled_win, SCROLL_ANCHO, SCROLL_ALTO);
	...
	for (i = 0; i < CLIST_INTERFACE_COLUMNS; i++) {
		...
		gtk_clist_set_column_auto_resize(GTK_CLIST(clist_interface), i, TRUE);
	}
  ...
  gtk_clist_column_titles_passive(GTK_CLIST(clist_interface));   
	...
	// ------------ refresh button  -----------------------
	vbox_aux  = gtk_box_new(GTK_ORIENTATION_VERTICAL, 0);
	button_refresh = gtk_button_new_with_label(" Refresh List ");

	gtk_widget_set_can_default(button_refresh, TRUE);	
...
void preferences::cargar_option_warning(GtkWidget *vbox)
{
	...
	vbox_warn = gtk_box_new(GTK_ORIENTATION_VERTICAL, BOX_PADDING);
	...
	gtk_widget_set_size_request(spinner_warntime, 40, 0);

	gtk_widget_set_tooltip_text(spinner_warntime, "seconds");
...
void preferences::cargar_prefered_interface(GtkWidget *vbox) 
{
	...
	gtk_widget_set_size_request(spinner_prefered, 40, 0);
...
void preferences::option_warn_sensitive(void)
{
	if(GTK_TOGGLE_BUTTON(button_option_warn)->active) {
		gtk_widget_set_sensitive(label_warntime, TRUE);
		gtk_widget_set_sensitive(spinner_warntime, TRUE);
		gtk_widget_set_sensitive(button_warn_beep, TRUE);
		gtk_widget_set_sensitive(label_warntime_spinner, TRUE);
		gtk_widget_set_sensitive(label_warntime_beep, TRUE);
	}
	else {
		gtk_widget_set_sensitive(label_warntime, FALSE);
		gtk_widget_set_sensitive(spinner_warntime, FALSE);
		gtk_widget_set_sensitive(button_warn_beep, FALSE);
		gtk_widget_set_sensitive(label_warntime_spinner, FALSE);
		gtk_widget_set_sensitive(label_warntime_beep, FALSE);
	}
}
